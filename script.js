// forked from os0x's "forked from: パーティクル時計" http://jsdo.it/os0x/4ObQ
...
function process() {
  ...
  if (nowDisp === CLOCK_VIEW) {
    // 時計
      var textSize = 50;
      var text = time;
      textWidth = textSize*7;
      textHeight = textSize;
      setX = _width/2 - textSize*3.3;
    ...
    // テキスト
      textSize = 40;
      text = '11 Graduation Live';
      textWidth = textSize * text.length;
      textHeight = textSize;
      setX = _width/2 - textSize * text.length/2 + 170;
      setY = _height/2 - textSize/2;
...
function timeDraw() {
  ...
  var endDate = new Date("January 1,2015 11:11:11");
  var left = endDate - nowDate;
  // カウントダウン終了
  if(left <= 0) {
    return "finish";
  }
